...
namespace DAL
{
    public class AdsDAO : PostContext
    {
        ...
        {
            Ad adsdao = db.Ads.First(x => x.ID == ID);
            string imagepath = adsdao.ImagePath;
            adsdao.isDeleted = true;
            adsdao.LastUpdateUserID = UserStatic.UserID;
            adsdao.DeleteDate = DateTime.Now;
            adsdao.LastUpdateDate = DateTime.Now;
            db.SaveChanges();
            return imagepath;
        }
